	...
	<div class="follow-container">
		<div class="followbody">
				<div id="follow" class="follow">
					<table class="table">
						<tbody>
						<c:if test="${noFollower  eq 'Y'}">
							팔로워 목록이 없습니다.
						</c:if>
						<c:forEach items="${followerList}" var="follower" step="1">
							<tr class="follow-tr">
								<!-- 팔로워/팔로우한 사람들의 프로필 사진과 소개(시작) -->
								<td class="follow-img"><img src="${follower.profile_img }"
									class="img-circle"/>
									<!-- 프로필 사진 출력 --></td>
								<td class="follow-name">
									<div id="id&name" class="id&name">
										<div id="id" class="id">${follower.follow_id }</div>
										<!-- id 출력 -->
										<div id="name" class="name">${follower.member_name }</div>
										<!-- 이름 출력 -->
									</div>
								</td>
							</tr>
